...
//    _joint->jvel_d =  maxspeed * sin( 2*M_PI * (1/f_period) * t.toSec());
  
//  else if (_joint->type == Z_INS_GOLD)
//    _joint->jvel_d =  0.2 * sin( 2*M_PI * (1/f_period) * t.toSec());
//
  else
    _joint->jvel_d = 0;
...
/**
*  update_joint_position_trajectory()
*     find next trajectory waypoint.
*     Sinusoidal position trajectory
*     \ingroup Control
*
*   /written by Andrew Razjigaev based on the above function
*/
int update_joint_position_trajectory(DOF *_joint) {
  //_trajectory *traj = &(trajectory[_joint->type]);

  //Allocate correct delta to each corresponding joint type that occurs

  //deltas_array is globally as 0 defined in rt_process_preempt.cpp!

  //Was originally _joint->jpos_d = deltas_array[...] + traj->startPos;

  //delta_array must be in radians and mm
  ...
  switch(_joint->type)
  {
    ...
    case SHOULDER_GOLD:
    {
      _joint->jpos_d += deltas_array[0];
      break;
    }
    case ELBOW_GOLD:
    {
      _joint->jpos_d += deltas_array[1];
      break;
    }    
    case Z_INS_GOLD:
    {
      _joint->jpos_d += deltas_array[2];
      break;
    }
    case TOOL_ROT_GOLD:
    {
      _joint->jpos_d += deltas_array[3];
      break;
    }
    case WRIST_GOLD:
    {
      _joint->jpos_d += deltas_array[4];
      break;
    }
    case GRASP1_GOLD:
    {
      _joint->jpos_d += deltas_array[5];
      break;
    }
    case GRASP2_GOLD:
    {
      _joint->jpos_d += deltas_array[6];
      break;
    }
    ...
    case SHOULDER_GREEN:
    {
      _joint->jpos_d += deltas_array[7];
      break;
    }
    case ELBOW_GREEN:
    {
      _joint->jpos_d += deltas_array[8];
      break;
    }    
    case Z_INS_GREEN:
    {
      _joint->jpos_d += deltas_array[9];
      break;
    }
    case TOOL_ROT_GREEN:
    {
      _joint->jpos_d += deltas_array[10];
      break;
    }
    case WRIST_GREEN:
    {
      _joint->jpos_d += deltas_array[11];
      break;
    }
    case GRASP1_GREEN:
    {
      _joint->jpos_d += deltas_array[12];
      break;
    }
    case GRASP2_GREEN:
    {
      _joint->jpos_d += deltas_array[13];
      break;
    }  
    default:
       _joint->jpos_d += 0; 
